...
- On approval, the admin decision endpoint **copies** the approved objects into a **public bucket `experience-photos-public`** (added in `20260910120000`) and sets `experiences.cover_image_url` / `gallery` to those public URLs. Private originals stay in `experience-photos` for audit. *(Originally D2 said copy into `catalog-images`; that bucket is `image/webp`-only "licensed stock photography", so approved host photos — jpeg/png/webp — get their own public bucket instead. Same single-public-surface property.)*

**Why copy, not signed-URL-always:** copy keeps the public surface one well-known bucket with stable CDN-friendly URLs. A takedown is one delete from that bucket + set the experience `archived`; the private original is not publicly reachable so it needs no urgent action. Signed-URL-always also unpublishes cleanly but couples every catalog image read to short-lived URL minting forever, and diverges from the existing catalog delivery model. Rejected the "public bucket, rely on nothing linking to unpublished photos" option outright — that is obscurity and it publicly hosts unreviewed uploads on our domain.
...
### N1 admin experience-review node — done (`20260910120000` + admin app)

- **Scope split kept, not collapsed.** New scope **`content:decide`** (9th). `content:manage` = triage + normalise taxonomy + **recommend**; `content:decide` = **commit** a publish/reject. Mirrors P2's `hosts:review` / `hosts:decide`. Founder holds both. Ripple: DB CHECK `staff_members_scopes_known`, `admin/src/lib/scopes.ts`, the founder bootstrap array, and the three test fixtures that build a founder row.
- **Schema** (`experience_review_workflow.sql`): `experience_review_decision` enum (same 7 values as P2's `host_review_decision`); `experience_reviews` append-only history (`experience_id, reviewer_id, from_status, to_status, decision, checklist, note, reason_code, created_at`); `experiences.reviewer_id`; RLS `SELECT` via `has_scope('content:manage')`, **no client write policy**. Plus a `content:manage` read policy on `experience_departures` for the detail screen, and a **public bucket `experience-photos-public`** (D2 target — `catalog-images` is webp-only stock, so approved host photos get their own public home; no client write policy).
- **Decision endpoint** (not a plpgsql RPC — copy-on-approve is a Storage op): `POST /api/experiences/[id]/decision`, `withAdmin("content:decide", …, {mutating})`. On `approve`: requires `categoryId` + `regionId` + `difficulty`; `ctx.db.storage.copy(path, path, {destinationBucket: "experience-photos-public"})` for each private gallery path; sets `gallery` / `cover_image_url` to the public URLs; `status → published`. On `reject` / `request_changes`: `status → draft`, reason mandatory, private originals retained. Every path: `experience_reviews` row + `ctx.audit` → `admin_audit_log` + `notifyExperienceDecision` (gated on `HOST_DECISION_COPY`, still null). `POST …/recommendation` is `content:manage`, no status change.
- **Screens**: `/experiences` queue (status filter, `pending_review` default, oldest-first, age badge) + `/experiences/[id]` detail (submitted listing as `<dl>`, departure, itinerary, photos via `…/photos/url` signed-URL endpoint, `ExperienceReviewPanel` with the taxonomy `<select>`s shown to a `content:decide` holder). Nav entry added to `AdminShell`.
