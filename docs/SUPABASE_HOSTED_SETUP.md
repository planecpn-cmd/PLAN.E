...
The bundle covers every migration in `supabase/migrations/` except
`0018_seed_experience_departures.sql` (which depends on the catalog seed and
runs separately, see below), including the service-role grants used by Edge
Functions. Regenerate it with `tool/build_schema_bundle.sh` after changing any
migration.
