...
## (f) Deploy the admin Worker: build-time env, both secrets, hostname binding

**`NEXT_PUBLIC_*` is inlined into the client bundle at `next build` time, not
read at runtime.** `wrangler secret put NEXT_PUBLIC_SUPABASE_ANON_KEY` and
`wrangler.jsonc`'s `vars.NEXT_PUBLIC_SUPABASE_URL` both only reach
`process.env` inside the Workers runtime for server-side code — they have
**zero effect** on the JS the browser actually downloads, because that JS
was already generated by `next build`, before any of that exists. This
already caused a real incident: `admin/.env.local` (git-ignored, local-dev
defaults) was loaded by Next.js during the production build, its values got
baked into the shipped bundle, and the deployed Worker's browser client
pointed at `127.0.0.1:54341` with the local demo anon key — a database that
doesn't exist outside a developer's own machine. The page still rendered
fine; only the actual Google OAuth redirect (which showed
`redirect_uri=http://127.0.0.1:54341/...`) revealed it.

`npm run deploy` now refuses to make this mistake again:

```sh
cd admin
export NEXT_PUBLIC_SUPABASE_URL=https://dtebgbrqynxahuzmbtbc.supabase.co
export NEXT_PUBLIC_SUPABASE_ANON_KEY=<the project's public anon key>
...
`npm run deploy` is now `require-prod-env.mjs && opennextjs-cloudflare
build && check-deploy-bundle.mjs && opennextjs-cloudflare deploy`:
`require-prod-env.mjs` refuses to build at all if `NEXT_PUBLIC_SUPABASE_URL`
/`NEXT_PUBLIC_SUPABASE_ANON_KEY` aren't explicitly exported (so `.env.local`
can never silently win), and `check-deploy-bundle.mjs` scans the actual
files `wrangler deploy` uploads for local-stack strings *and* positively
asserts the real hosted project ref is present, refusing to deploy either
way it can go wrong.

- `SUPABASE_SERVICE_ROLE_KEY` — the real service-role key. This one is a
  genuine Worker secret (server-only, `admin/src/lib/service-role.ts` is
  the only file that reads it) — `docs/ADMIN_ACCESS_VERIFICATION.md`
  already confirmed the built output doesn't leak it into the client.
...
The hostname (`admin.planenepal.com`) is already committed in
`admin/wrangler.jsonc` — nothing to configure there.

**Success looks like:** `npm run deploy` prints `check-deploy-bundle:
clean...` before it deploys, and ends with a deployed Worker URL. Then
verify the way step (h) below describes — a 200 response and a rendered
form are not enough on their own; that's exactly what looked fine during
the incident this section now guards against.

**If it fails:** a failed deploy doesn't take down anything — Cloudflare
keeps serving the last successful deploy until a new one succeeds. If
`require-prod-env.mjs` or `check-deploy-bundle.mjs` fails, that's the
guard working — fix the environment, don't bypass the script. If `next
build` itself fails, that should already have been caught locally (`npm
run build` is part of the check suite); if it passes locally and fails
here, it's almost always a missing secret or an account/permissions issue
with Cloudflare, not the code.
...
## A pattern worth naming: "renders" is not "verified"

This is the second time a step that looked complete turned out to verify
nothing real:

- **Seed auth fixtures** (`supabase/tests/seed_fixture_auth.test.mjs`):
  `seed_test.sql` inserted rows straight into `auth.users`, which the SQL
  suite (authenticates by fabricating `request.jwt.claims`, never through
  GoTrue) and the rest of the edge suite (creates its own throwaway users)
  both happily passed against. It surfaced only when a human tried to set a
  real password on a seed fixture and GoTrue 500'd on a NULL token column
  neither suite ever exercised.
- **The admin Worker pointing at localhost** (this section): the login
  page returned 200 and rendered a real form with the right title,
  regardless of which Supabase project its client JS was wired to. Only an
  actual OAuth redirect — or grepping the shipped bundle — revealed it.

Both times, the check that would have caught it was cheap and the check
that didn't catch it was expensive-sounding (a full test suite; a rendered
screenshot) but structurally couldn't see the failure. **When verifying a
deploy step, ask what the user's browser actually *does*, not whether a
page returns 200 or a form is present.** For auth-adjacent surfaces
specifically: trigger the real client-side flow (a login attempt, an OAuth
redirect) and read where it actually goes, stopping short of entering
credentials rather than skipping the step entirely.
...
0. **Before signing in**, confirm the backend without authenticating: on
   `admin.planenepal.com`, click "Continue with Google" and read the
   `redirect_uri` in the resulting Google URL (browser address bar or dev
   tools network tab) — it must be
   `https://dtebgbrqynxahuzmbtbc.supabase.co/auth/v1/callback`. Stop there;
   don't complete the sign-in. This is the check that actually would have
   caught the `.env.local`-in-production incident in step (f) — the login
   page itself renders identically either way.
